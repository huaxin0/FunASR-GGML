...
class FunAsr(mode):
    ...
    def set_gguf_parameters(self):
        ...
        rope_theta = self.find_hparam("rope_theta", config=self.llm_config)
        self.gguf_writer.add_rope_freq_base(float(rope_theta) if rope_theta is not None else 1000000.0)
        # 安全获取 head_dim
        ...
        # 安全获取 intermediate_size
        intermediate_size = self.find_hparam("intermediate_size", config=self.llm_config)
    ...
    def write_one_tensor(self, data_torch, name):
        ...
        # 判决法则：只量化 2D 权重。跳过 1D (bias/norm) 和 Embedding/FSMN 等敏感层
        should_quantize = (
            self.ftype == gguf.GGMLQuantizationType.Q8_0 and
            n_dims == 2 and 
            "norm" not in name and 
            "fsmn_block" not in name and
            "embed" not in name and 
            data_torch.shape[-1] % 32 == 0
        )
        
        if should_quantize:
            packed_data = self.quantize_q8_0(data_torch)
                             
            byte_shape = list(data_torch.shape)
            byte_shape[-1] = (byte_shape[-1] // 32) * 34
            packed_data = packed_data.reshape(byte_shape)
            ...
            print(f" Quantized {name} | shape {data_torch.shape} | {old_dtype} -> Q8_0")
            return
            
        # ---- 原始的高精度保留逻辑 (针对 Bias / Norm 等) ----
        if data_torch.dtype == torch.bfloat16:
            data = data_torch.view(torch.uint16).numpy()  # 保留原始比特
            target_qtype = gguf.GGMLQuantizationType.BF16
            self.gguf_writer.add_tensor(name, data, raw_dtype=target_qtype)
            return
        ...
        data = data_torch.numpy()
        if self.ftype == gguf.GGMLQuantizationType.F32 and data.dtype == np.float16 and 'fsmn_block.weight' not in name:
            data = data.astype(np.float32)
    ...
    def quantize_q8_0(self, tensor: torch.Tensor) -> np.ndarray:
        # 确保转成 float32 并拉到 CPU
        data = tensor.float().cpu().numpy()
        
        # GGML Q8_0 要求 block size 为 32
        assert data.shape[-1] % 32 == 0, f"最后一个维度 {data.shape[-1]} 不是 32 的倍数"
        ...
        qs = np.round(blocks / d[:, None])
        qs = np.clip(qs, -127, 127).astype(np.int8)
        
        # 2字节的 d (float16) + 32字节的 qs (int8) = 34 字节
        d_f16 = d.astype(np.float16).reshape(-1, 1).view(np.uint8) # shape: (num_blocks, 2)
        qs_uint8 = qs.view(np.uint8)                # shape: (num_blocks, 32)
        
        packed = np.concatenate([d_f16, qs_uint8], axis=1)
...
if __name__ == "__main__":
    ...
    model_path = Path("/home/hua/.cache/modelscope/hub/models/FunAudioLLM/Fun-ASR-Nano-2512")
    out_path = Path("./FunAsr_q8.bin")
    

    asr = FunAsr(model_path,gguf.GGMLQuantizationType.Q8_0, out_path, False)
